#! /usr/bin/env deno test --allow-read --unstable test_unstable.ts

import { getTestClient } from "./tests/helpers.ts";

import { TEST_CONNECTION_PARAMS } from "./tests/constants.ts";
import { Client } from "./client.ts";
import { assertEquals } from "./test_deps.ts";
import { DEFAULT_SETUP } from "./tests/constants.ts";
import { QueryResult } from "./query.ts";

const CLIENT = new Client({
  ...TEST_CONNECTION_PARAMS,
  conn: () => Deno.connect({ path: "/tmp/.s.PGSQL.5432", transport: "unix" }),
});
